...
export default function Users() {
  ...
  let [listUsers, setListUsers] = useState<any>([])
  ...
  const handleUpdateTable = (user: any, flag = false) => {
    if (flag) {
      let userIndex = listUsers.findIndex((item: any) => item.id === user.id)
  ...
  const handleSearch = debounce(async (event: any) => {
    ...
    if (term) {
      ...
      cloneListUser = cloneListUser.filter((item: any) => item.email.includes(term))
  ...
  const getUserExport = (event: any, done: any) => {
    let result = []
  ...
  const handleSort = (sortBy: any, sortField: any) => {
    setSortBy(sortBy)
